...
func loadEnvironmentVariables() {
	...
	updateFrequencyInt, _ := strconv.Atoi(LookupEnvOrString("UPDATE_FREQUENCY", "300"))
	scrapeIntervalInt, _ := strconv.Atoi(LookupEnvOrString("SCRAPE_INTERVAL", "60"))
	prometheusBindPortInt, _ := strconv.Atoi(LookupEnvOrString("PROMETHEUS_BIND_PORT", "60000"))
	flag.Int64Var(&updateFrequency, "update-frequency", int64(updateFrequencyInt), "The time in seconds, that takes between each apps update call")
	flag.Int64Var(&scrapeInterval, "scrape-interval", int64(scrapeIntervalInt), "The time in seconds, that takes between Prometheus scrapes")
	flag.IntVar(&prometheusBindPort, "prometheus-bind-port", prometheusBindPortInt, "The port to bind to for prometheus metrics")
